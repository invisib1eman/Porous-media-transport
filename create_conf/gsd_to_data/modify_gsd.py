...
for tag in range(config.particles.N):
    unwrap_position[tag] = config.particles.position[tag] + np.multiply(config.particles.image[tag], config.configuration.box[:-3])
"""
...
snap.particles.typeid = config.particles.typeid + 1
snap.particles.mass = [1.0]*config.particles.N 
snap.particles.charge = [0.0]*config.particles.N 
snap.particles.velocity = config.particles.velocity
#snap.particles.image = np.zeros_like(image)
...
snap.bonds.typeid = config.bonds.typeid + 1
